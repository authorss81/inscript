...
def test_unexpected_char():
    try:
        tokenize("let x = `5")  # backtick is still invalid
        assert False, "Should have raised LexerError"
    except LexerError as e:
        assert "Unexpected character" in e.message
test("Error: unexpected character '`'", test_unexpected_char)

# ─── Test: single & is valid bitwise AND ──────
def test_single_ampersand():
    toks = tokenize("x & y")
    assert any(t.type == TT.BIT_AND for t in toks), f"Expected BIT_AND token in {toks}"
test("Single '&' lexes as BIT_AND token", test_single_ampersand)
