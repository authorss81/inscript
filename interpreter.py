...
class Interpreter(Visitor):
    ...
    def _register_builtins(self):
        ...
        native_fns = {
            ...
            "channel":     lambda cap=0: {"_type":"channel","_buf":[],"_cap":cap},
            "chan_send":    lambda ch, v: ch["_buf"].append(v),
            "chan_recv":    lambda ch: ch["_buf"].pop(0) if ch["_buf"] else None,
            ...
            "Ok":          lambda v=None: {"_ok": v},
            "Err":         lambda e="error": {"_err": e},
    ...
    def visit_GeneratorFnDecl(self, node) -> Any:
        ...
        def _make_generator(*arg_vals):
            """Creates a Python generator that runs the InScript body, yielding on YieldSignal."""
            def _body_gen():
                interp._push(f"gen:{node.name}")
                # Bind parameters
                for i, param in enumerate(node.params):
                    val = arg_vals[i] if i < len(arg_vals) else (
                        interp.visit(param.default) if param.default else None)
                    interp._env.define(param.name, val)
                try:
            ...
            return InScriptGenerator(name=node.name, python_gen=_body_gen())
    ...
    def visit_EnumDecl(self, node: EnumDecl) -> Any:
        ...
        for variant in node.variants:
            if variant.fields:
                ...
                vname = variant.name
                def _make_adt(*args, _vname=vname, _fnames=field_names):
                    d = {"_variant": _vname, "_enum": node.name}
                    for i, fname in enumerate(_fnames):
                        d[fname] = args[i] if i < len(args) else None
                    return d
                enum_vals[variant.name] = _make_adt
                # Also expose variant factory as a bare name for match patterns:
                # `case Circle(r)` needs `Circle` in scope
    ...
    def visit_DestructureDecl(self, node: DestructureDecl) -> Any:
        ...
        if node.is_object:
            # Object destructuring: let {x, y: renamed} = point
